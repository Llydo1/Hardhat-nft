...
developmentChains.includes(network.name) &&
    describe("Dynamic Svg Nft contract testing", () => {
        ...
        beforeEach(async () => {
            ...
            v3AggregatorMock = await ethers.getContract("MockV3Aggregator", deployer);
        });

        describe("Constructor", () => {
            it("Initialize name and symbol correctly", async () => {
            ...
            it("Initialize SVGs correctly", async () => {
            ...
            it("Initialize price feed address correctly", async () => {
                const priceFeed = await dynamicSvgNft.getPriceFeed();
                assert.equal(priceFeed, v3AggregatorMock.address);
            });
        ...
        describe("mintNft", () => {
            ...
            it("revert if there is no token URI", async () => {
                await expect(dynamicSvgNft.tokenURI(15)).to.be.revertedWith(
                    "DynamicSgvNft__TokenURINotFound"
                );
            });
        });
    });
